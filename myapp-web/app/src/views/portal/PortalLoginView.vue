  ...
  <div class="flex items-center justify-center min-h-[calc(100vh-3.5rem)] px-6 py-12">
    <div class="w-full max-w-sm">
      <div class="text-center mb-8">
        ...
        <p class="text-sm text-white/60 mt-2">予約・決済方法の管理</p>
      </div>
...
import { fetchPortalMe, portalLogin, portalLogout } from '../../lib/portalApi'
...
async function submit() {
  ...
  errorMsg.value = ''
  try {
    await portalLogin(username.value, password.value)
    const me = await fetchPortalMe()
    if (!me.is_customer) {
      portalLogout()
      errorMsg.value = 'このアカウントはポータルをご利用いただけません。'
      return
    }
    sessionStorage.setItem('portal_username', me.username)
